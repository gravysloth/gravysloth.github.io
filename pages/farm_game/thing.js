class Thing {
  ...
  collision() {
    for (let i = 0; i < ThingList.length; i++) {
      ...
      if (
        dist(thing.x, thing.y, this.x, this.y) <= thing.radius + this.radius &&
        this !== thing
      ) {
        return thing;
      }
  ...
  collisionObject(thing) {
    for (let i = 0; i < ThingList.length; i++) {
      ...
      if (
        dist(test.x, test.y, this.x, this.y) <= test.radius + this.radius &&
        this !== test &&
  ...
  sprite() {
    image(
      this.image,
      floor(this.x - this.image.width / 2 + this.dx + 0.5 + this.imageOffsetX),
